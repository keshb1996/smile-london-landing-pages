  ...
  >
      <div className="dental-section">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          ...
          return <div key={index} className="flex flex-col items-center text-center space-y-3 p-4 bg-white rounded-lg shadow-sm">
                <div className="bg-dental-gold/10 rounded-full p-3">
                  <IconComponent className="h-6 w-6 text-dental-gold" />
                </div>
                <div>
                  <h3 className="font-semibold text-gray-800 mb-1">{benefit.title}</h3>
                  {benefit.value && <p className="text-sm text-gray-600">{benefit.value}</p>}
                </div>
              </div>;
        })}
        </div>

...
const AllOnFourFinancing = () => {
  ...
  const financingOptions = [{
    ...
    monthly: "Pay in Full",
    period: "Immediate",
    features: ["10% discount for full payment", "No financing required", "Immediate treatment start", "Complete in single visit"],
    highlight: "Best Value"
  }];
  return <div>
      <section ref={mainRef} className={`bg-white ${mainVisible ? 'animate-fade-up' : 'opacity-0 translate-y-[30px]'}`}>
      <div className="dental-section">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {financingOptions.map((option, index) => <div key={index} className="relative bg-dental-gray rounded-2xl p-8 hover:shadow-dental-lg transition-all duration-300 group">
              {/* Highlight badge */}
              {option.highlight && <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
              ...
              <div className="space-y-4 mb-8">
                {option.features.map((feature, featureIndex) => <div key={featureIndex} className="flex items-center gap-3">

...
const AllOnFourTestimonials = () => {
  ...
  return <section className="bg-dental-black text-dental-black-foreground">
      <div className="dental-section">
        ...
        <div className="relative max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-4 gap-8 items-center">
              ...
              <div className="text-center">
                ...
                <div className="flex justify-center gap-1 mt-2">
                  {[...Array(testimonials[currentSlide].rating)].map((_, i) => <Star key={i} className="w-5 h-5 fill-primary text-primary" />)}
                </div>
              </div>
          ...
          <div className="flex justify-center items-center gap-4 mt-8">
            <Button variant="outline" size="icon" onClick={prevSlide} className="rounded-full bg-white border-white text-foreground hover:bg-primary hover:text-primary-foreground">
              <ChevronLeft className="w-5 h-5" />
            </Button>
            
            {/* Dots indicator */}
            <div className="flex gap-2">
              {testimonials.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`w-3 h-3 rounded-full transition-all ${index === currentSlide ? 'bg-primary' : 'bg-white/30'}`} />)}
            </div>
            
            <Button variant="outline" size="icon" onClick={nextSlide} className="rounded-full bg-white border-white text-foreground hover:bg-primary hover:text-primary-foreground">
